...
public class ProtoWriterReaderTest
{
    ...
    public void TestEncodeTwo32VarIntUnsafe_SingleByteValues()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(buffer.WrittenMemory.Length, Is.EqualTo(2)); 
            Assert.That(isCompleted, Is.True);
        });
    }
    ...
    public void TestVarInt_MaxBytesPerType()
    {
        ...
        writer.EncodeVarInt((byte)0xFF);           
        writer.EncodeVarInt((ushort)0xFFFF);       
        writer.EncodeVarInt((uint)0xFFFFFFFF);     
        writer.EncodeVarInt((ulong)0xFFFFFFFFFFFFFFFF); 
        writer.Flush();
    ...
    {
        var buffer = new ArrayBufferWriter<byte>(1); 
        var writer = new ProtoWriter(buffer);
    ...
    {
        byte[] data = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF }; 
