    ...
    where TKey : notnull
{
    ...
    public override void Write(ProtoWriter writer, object target)
    {
        ...
        foreach (var (key, value) in map)
        {
            ...
            writer.EncodeVarInt(KeyEffectiveConverter.Measure(1, KeyWireType, key) + ValueEffectiveConverter.Measure(2, ValueWireType, value) + 2); 
    ...
    public override int Measure(object target)
    {
        ...
        int size = ProtoHelper.GetVarIntLength(tag) * (map.Count - 1) + 2 * map.Count; 
        foreach (var (key, value) in map)
        {
