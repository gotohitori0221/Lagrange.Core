...
public ref struct ProtoReader
{
    ...
    public unsafe T DecodeVarIntUnsafe<T>() where T : unmanaged, INumber<T>
    {
        if (sizeof(T) <= 4) 
        {
            ...
            _offset += (BitOperations.TrailingZeroCount(msbs) + 1) >> 3; 
        ...
        else
        {
            ...
            int len0 = BitOperations.TrailingZeroCount(msbs0) + 1; 
        ...
        where TU : unmanaged, INumber<TU>
    {
        ...
        if (sizeof(TT) <= 4 && sizeof(TU) <= 4) return DecodeTwo32VarIntUnsafe<TT, TU>(src); 
