...
public class LoginService(IHostEnvironment environment, ILogger<LoginService> logger,LagrangeConfiguration configuration, BotContext lagrange, ICaptchaResolver captchaResolver) : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken ct)
    {
        ...
        _logger.LogProtocolVersion(_lagrange.AppInfo.CurrentVersion);
    ...
    private async Task OnQrCode(BotContext lagrange, BotQrCodeEvent @event)
    {
        ...
        Console.Write(QrCode.EncodeText(@event.Url, QrCode.Ecc.Low).ToAscii(_compatibleQrCode));
    ...
    private void OnQrCodeQuery(BotContext lagrange, BotQrCodeQueryEvent @event)
    {
        var level = @event.State switch
        {
            ...
            TransEmpState.WaitingForConfirm or
            TransEmpState.Confirmed => LogLevel.Information,
            _ => LogLevel.Error,
        };
        if (@event.Uin != 0)
            _logger.LogQrCodeQueryStateWithUin(level, @event.State, @event.Uin);
        else
...
public static partial class LagrangeLoginServiceLoggerExtension
{
    ...
    [LoggerMessage("QR code scan state: {State} Uin: {Uin}")]
    public static partial void LogQrCodeQueryStateWithUin(this ILogger<LoginService> logger, LogLevel level, TransEmpState state, long uin);
    ...
    [LoggerMessage(LogLevel.Information, "Protocol Version: {Version}")]
    public static partial void LogProtocolVersion(this ILogger<LoginService> logger, string version);

    [LoggerMessage(LogLevel.Information, "Bot Uin: {Uin}")]
    public static partial void LogLoginSuccess(this ILogger<LoginService> logger, long uin);
}
