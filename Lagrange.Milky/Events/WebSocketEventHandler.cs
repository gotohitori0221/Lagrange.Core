...
public sealed class WebSocketEventHandler : IHttpHandler, IGenericEventHandler, IDisposable
{
    ...
    public WebSocketEventHandler(IServiceScopeFactory scopeFactory, ILogger<WebSocketEventHandler> logger, MilkyConfiguration configuration, MilkyWebSocketEventConfiguration wsConfiguration, BotContext lagrange)
    {
        ...
        _logger.LogServerStarted(configuration.HttpServer.Host, configuration.HttpServer.Port);
    ...
    private async Task SendAllAsync(byte[] payload, CancellationToken ct = default)
    {
        ...
        await Task.WhenAll(_wss.Select(async kvp =>
        {
    ...
    public async Task OnEvent<TEvent>(BotContext lagrange, TEvent @event) where TEvent : EventBase
    {
        ...
        foreach (var converter in scope.ServiceProvider.GetServices<IEventConverter<TEvent>>())
        {
            if (!converter.CanConvert(@event)) continue;
...
{
    [LoggerMessage(LogLevel.Information, "The WebSocket event server is started at {Host}:{Port}")]
    public static partial void LogServerStarted(this ILogger<WebSocketEventHandler> logger, string host, ushort port);
    ...
    [LoggerMessage(LogLevel.Information, "Connect({Id})")]
    public static partial void LogConnect(this ILogger<WebSocketEventHandler> logger, Guid id);
    ...
    [LoggerMessage(LogLevel.Information, "Disconnect({Id})")]
    public static partial void LogDisconnect(this ILogger<WebSocketEventHandler> logger, Guid id);

    [LoggerMessage(LogLevel.Error, "DisconnectException({Id})")]
