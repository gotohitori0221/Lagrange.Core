...
public sealed class GetMessageHandler(BotContext lagrange, MessageCache cache, MessageStore store, MilkyConverter converter) : IApiHandler<GetMessageHandler.Request, GetMessageHandler.Result>
{
    ...
    public async ValueTask<MilkyApiResponse<Result>> HandleAsync(Request request, CancellationToken ct)
    {
        ...
        var message = _cache.Get(messageType, request.PeerId, (ulong)request.MessageSeq)
            ?? _store.Get(messageType, request.PeerId, (ulong)request.MessageSeq)
            ?? (messageType switch
            {
