...
public sealed class SetNicknameHandler(BotContext lagrange) : INoResultApiHandler<SetNicknameHandler.Request>
{
    ...
    public async ValueTask<MilkyApiResponse> HandleAsync(Request request, CancellationToken ct)
    {
        ...
        string url = $"https://user.qzone.qq.com/proxy/domain/base.qzone.qq.com/cgi-bin/baseinfo/set_nick_name?g_tk={bkn}";
        ...
        if (body?.Code != 0)
            return new MilkyApiResponse(-500, $"Failed to set nickname, response: {raw}");

        if (_lagrange.Keystore.BotInfo != null)
        {
            _lagrange.Keystore.BotInfo.Name = request.Nickname;
        }
    ...
    {
        [JsonPropertyName("new_nickname")] public required string Nickname { get; init; }
    }
