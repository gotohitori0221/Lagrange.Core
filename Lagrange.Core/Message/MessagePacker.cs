...
internal class MessagePacker(BotContext context)
{
    ...
    public async Task<BotMessage> Parse(CommonMessage msg)
    {
        ...
        if (ParsePrivateFile(msg, contentHead.Type) is { } file)
        {
            message.Entities.Add(file);
        }

        
        
        
        for (int i = 0; i + 1 < message.Entities.Count; i++)
        {
            if (message.Entities[i] is ReplyEntity && message.Entities[i + 1] is MentionEntity)
            {
                message.Entities.RemoveAt(i + 1);
                while (i + 1 < message.Entities.Count &&
                       message.Entities[i + 1] is TextEntity text && string.IsNullOrWhiteSpace(text.Text))
                {
                    message.Entities.RemoveAt(i + 1);
                }
                break;
        ...
        if (message.Entities.Any(e => e is MarkdownEntity))
        {
            for (int i = 0; i + 1 < message.Entities.Count; i++)
            {
                if (message.Entities[i] is TextEntity && message.Entities[i + 1] is MarkdownEntity)
                {
    ...
    private static FileEntity? ParsePrivateFile(CommonMessage msg, int type)
    {
        var file = msg.MessageBody?.RichText.NotOnlineFile;

        
        if (file is null && type == 529 && msg.MessageBody?.MsgContent is { Length: > 0 } content)
        {
            file = ProtoHelper.Deserialize<FileExtra>(content.Span).File;
        }

        if (file is not { FileUuid.Length: > 0 }) return null;
        ...
        return new FileEntity
        {
            ...
            FileMd5 = file.FileMd5 is { Length: > 0 } md5 ? Convert.ToHexString(md5) : string.Empty,
            FileHash = file.FileIdCrcMedia,
        };
    }
    ...
    private RecordEntity? ParsePttRichText(RichText richText)
    {
        ...
        outer.PushSequence();
        outer.WriteInteger(1); 
        outer.WriteOctetString(innerSpan[..length]); 
        outer.WriteOctetString(ReadOnlySpan<byte>.Empty);   
        outer.PopSequence();
