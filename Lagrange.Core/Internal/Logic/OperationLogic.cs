...
internal class OperationLogic(BotContext context) : ILogic
{
    ...
    {
        if (await context.CacheContext.ResolveGroupMemberUidAsync(groupUin, targetUin) is not { } uid) return false;
    ...
    {
        if (await context.CacheContext.ResolveGroupMemberUidAsync(groupUin, targetUin) is not { } uid)
            throw new InvalidTargetException(targetUin);
    ...
    public async Task<string> GroupFSCreateFolder(long groupUin, string name, string parentFolderId = "/")
    {
        var response = await context.EventContext.SendEvent<GroupFSCreateFolderEventResp>(new GroupFSCreateFolderEventReq(groupUin, name, parentFolderId));
    ...
    public async Task GroupFSPersist(long groupUin, string fileId)
    {
        await context.EventContext.SendEvent<GroupFSPersistEventResp>(new GroupFSPersistEventReq(groupUin, fileId));
    }

    public async Task<(ulong Sequence, long Time, string FileId)> SendFriendFile(long targetUin, Stream fileStream, string? fileName)
    {
        ...
        return (sequence, sendResult.SendTime, result.FileId);
    }
    ...
    public async Task<bool> DeleteFriend(string targetUid, bool block)
    {
        var result = await context.EventContext.SendEvent<DeleteFriendEventResp>(new DeleteFriendEventReq(targetUid, block));
        return result.ResultCode == 0;
    }

    public async Task<bool> SetFriendRequest(string targetUid, bool accept)
    {
        var result = await context.EventContext.SendEvent<SetFriendRequestEventResp>(new SetFriendRequestEventReq(targetUid, accept));
        return result.ResultCode == 0;
    }

    public async Task<bool> SendProfileLike(string targetUid, uint count)
    {
        var result = await context.EventContext.SendEvent<FriendLikeEventResp>(new FriendLikeEventReq(targetUid, count));
        return result.ResultCode == 0;
    }

    public async Task<bool> GroupSetAdmin(long groupUin, string targetUid, bool isAdmin)
    {
        var result = await context.EventContext.SendEvent<GroupSetAdminEventResp>(new GroupSetAdminEventReq((uint)groupUin, targetUid, isAdmin));
        return result.ResultCode == 0;
    }

    public async Task<bool> SetGroupAvatar(long groupUin, Stream imageStream)
    {
        ...
        var extendInfo = ProtoHelper.Serialize(extra);
        return await context.HighwayContext.UploadFile(imageStream, 3000, extendInfo);
    }

    public async Task<bool> MarkMessageAsRead(long groupUin, uint startSequence)
    {
        await context.EventContext.SendEvent<MarkReadedEventResp>(new MarkReadedEventReq
        {
            GroupUin = (uint)groupUin,
            StartSequence = startSequence,
        });
        return true;
    }

    public async Task<bool> MarkC2CMessageAsRead(string targetUid, uint startSequence, uint time)
    {
        await context.EventContext.SendEvent<MarkReadedEventResp>(new MarkReadedEventReq
        {
            TargetUid = targetUid,
    ...
    public async Task GroupFSRenameFile(long groupUin, string fileId, string parentFolderId, string newFileName)
    {
        await context.EventContext.SendEvent<GroupFSRenameFileEventResp>(new GroupFSRenameFileEventReq(groupUin, fileId, parentFolderId, newFileName));
    }

    public async Task<(List<string> FriendUids, List<uint> GroupUins)> FetchPins()
    {
        var result = await context.EventContext.SendEvent<FetchPinsEventResp>(FetchPinsEventReq.Instance);
        return (result.FriendUids, result.GroupUins);
    }
}
