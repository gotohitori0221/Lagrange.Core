...
[EventSubscribe<GroupFSRenameFileEventReq>(Protocols.All)]
[Service("OidbSvcTrpcTcp.0x6d6_4")]
internal class GroupFSRenameFileService : OidbService<GroupFSRenameFileEventReq, GroupFSRenameFileEventResp, D6D6ReqBody, D6D6RspBody>
{
    protected override uint Command => 0x6d6;
    ...
    protected override Task<D6D6ReqBody> ProcessRequest(GroupFSRenameFileEventReq request, BotContext context)
    {
        return Task.FromResult(new D6D6ReqBody
        {
    ...
    protected override Task<GroupFSRenameFileEventResp> ProcessResponse(D6D6RspBody response, BotContext context)
    {
        var rename = response.RenameFileRsp;
        if (rename.Int32RetCode != 0) throw new OperationException((int)rename.Int32RetCode, rename.StrRetMsg);

        return Task.FromResult(GroupFSRenameFileEventResp.Default);
    }
}

[EventSubscribe<GroupFSPersistEventReq>(Protocols.All)]
[Service("OidbSvcTrpcTcp.0x6d9_0")]
internal class GroupFSPersistService : OidbService<GroupFSPersistEventReq, GroupFSPersistEventResp, D6D9ReqBody, D6D9RspBody>
{
    ...
    protected override Task<D6D9ReqBody> ProcessRequest(GroupFSPersistEventReq request, BotContext context)
    {
    ...
    protected override Task<GroupFSPersistEventResp> ProcessResponse(D6D9RspBody response, BotContext context)
    {
        var persist = response.TransFileRsp;
        if (persist is not null && persist.RetCode != 0) throw new OperationException((int)persist.RetCode, persist.RetMsg);

        return Task.FromResult(new GroupFSPersistEventResp());
    }
}
