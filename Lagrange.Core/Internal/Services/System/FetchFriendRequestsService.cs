...
internal class FetchFriendRequestsService : OidbService<FetchFriendRequestsEventReq, FetchFriendRequestsEventResp, D5CFReqBody, D5CFRspBody>
{
    ...
    protected override async Task<FetchFriendRequestsEventResp> ProcessResponse(D5CFRspBody response, BotContext context)
    {
        var success = response.SuccessRead ?? throw new OperationException(-1, "Friend request list was not returned");
        if (success.All == null) return new FetchFriendRequestsEventResp([]);

        List<BotFriendRequest> requests = [];
        foreach (var request in success.All)
        {
            requests.Add(new BotFriendRequest(
                await ResolveUin(request.SelfUid, context),
                request.SelfUid,
                await ResolveUin(request.FriendUid, context),
                request.FriendUid,
                request.State,
        ...
        return new FetchFriendRequestsEventResp(requests);
    }

    private static async Task<long> ResolveUin(string uid, BotContext context) =>
        uid == context.Keystore.Uid ? context.BotUin : await context.CacheContext.ResolveUinAsync(uid);
}
