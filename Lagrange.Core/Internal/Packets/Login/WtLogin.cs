...
internal class WtLogin : StructBase
{
    ...
    public ReadOnlyMemory<byte> BuildQrlogin19(byte[] k) 
    {
        using var writer = new BinaryPacket(stackalloc byte[300]);
        writer.Write<ushort>(0);
        writer.Write(AppInfo.AppId);
        writer.Write(Keystore.Uin);
        writer.Write(k, Prefix.Int16 | Prefix.LengthOnly); 
    ...
    public ReadOnlyMemory<byte> BuildQrlogin20(byte[] k) 
    {
        using var writer = new BinaryPacket(stackalloc byte[300]);
        writer.Write<ushort>(0);
        writer.Write(AppInfo.AppId);
    ...
    public ReadOnlyMemory<byte> BuildQrlogin22(byte[] k)
    {
        ...
        writer.Write(AppInfo.AppId);
        writer.Write(k, Prefix.Int16 | Prefix.LengthOnly); 
        writer.Write(_context.Keystore.Uin); 
        writer.Write<byte>(8);
        writer.Write(Keystore.WLoginSigs.A2, Prefix.Int16 | Prefix.LengthOnly);
        ...
        bool useWtSession = false) 
    {
        ...
        writer.Write((byte)2);
        writer.Write((short)0); 
        writer.Write(AppInfo.AppClientVersion); 
        writer.Write(0); 
        ...
        bool useWtSession = false)
    {
        ...
        reqBody.Write((byte)2); 
        reqBody.EnterLengthBarrier<short>();
        reqBody.Write(command);
        reqBody.Skip(21);
        reqBody.Write((byte)3); 
        reqBody.Write((short)0x00); 
        reqBody.Write((short)0x32); 
        reqBody.Write(0); 
        reqBody.Write(Keystore.Uin); 
        reqBody.Write(tlv);
        reqBody.Write((byte)3); 
        ...
        writer.Write(Convert.ToByte(encrypt)); 
        writer.Write((ushort)reqSpan.Length);
        writer.Write(AppInfo.AppId);
        writer.Write((uint)0x72); 
        writer.Write(encrypt ? Keystore.WLoginSigs.St : ReadOnlySpan<byte>.Empty, Prefix.Int16 | Prefix.LengthOnly); 
        writer.Write(ReadOnlySpan<byte>.Empty, Prefix.Int8 | Prefix.LengthOnly); 
    ...
    public ReadOnlySpan<byte> Parse(ReadOnlySpan<byte> input, out ushort command)
    {
        ...
        var encrypted = reader.ReadBytes()[..^1]; 
