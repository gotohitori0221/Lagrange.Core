...
public static class OperationExt
{
    ...
    public static Task<bool> DeleteFriend(this BotContext context, string targetUid, bool block = false) =>
        context.EventContext.GetLogic<OperationLogic>().DeleteFriend(targetUid, block);

    public static Task<bool> SetFriendRequest(this BotContext context, string targetUid, bool accept) =>
        context.EventContext.GetLogic<OperationLogic>().SetFriendRequest(targetUid, accept);

    public static Task<bool> SendProfileLike(this BotContext context, string targetUid, uint count = 1) =>
        context.EventContext.GetLogic<OperationLogic>().SendProfileLike(targetUid, count);

    public static Task<bool> GroupSetAdmin(this BotContext context, long groupUin, string targetUid, bool isAdmin) =>
        context.EventContext.GetLogic<OperationLogic>().GroupSetAdmin(groupUin, targetUid, isAdmin);

    public static Task<bool> SetGroupAvatar(this BotContext context, long groupUin, Stream imageStream) =>
        context.EventContext.GetLogic<OperationLogic>().SetGroupAvatar(groupUin, imageStream);

    public static Task<bool> MarkMessageAsRead(this BotContext context, long groupUin, uint startSequence) =>
        context.EventContext.GetLogic<OperationLogic>().MarkMessageAsRead(groupUin, startSequence);

    public static Task<bool> MarkC2CMessageAsRead(this BotContext context, string targetUid, uint startSequence, uint time) =>
        context.EventContext.GetLogic<OperationLogic>().MarkC2CMessageAsRead(targetUid, startSequence, time);

    public static Task GroupFSRenameFile(this BotContext context, long groupUin, string fileId, string parentFolderId, string newFileName) =>
        context.EventContext.GetLogic<OperationLogic>().GroupFSRenameFile(groupUin, fileId, parentFolderId, newFileName);
