...
public class BotGroupMemberDecreaseEvent(long groupUin, long userUin, long? operatorUin) : EventBase
{
    ...
    public override string ToEventMessage()
    {
        return OperatorUin == null
            ? $"{nameof(BotGroupMemberDecreaseEvent)}: GroupUin={GroupUin}, UserUin={UserUin}"
            : $"{nameof(BotGroupMemberDecreaseEvent)}: GroupUin={GroupUin}, UserUin={UserUin}, OperatorUin={OperatorUin}";
    }
}
