...
internal class Sha1Stream
{
    ...
    private unsafe void Transform(byte* data)  
    {
    ...
    public unsafe void Update(byte[] data, int len)  
    {
        fixed (byte* ptr = data) UpdateInternal(ptr, len);
    }
    
    public unsafe void Update(Span<byte> data)  
    {
    ...
    public void Hash(Span<byte> digest, bool bigEnding) 
    {
        if (bigEnding)
        {
            for (int i = 0; i < _state.Length; i++)
            {
                BinaryPrimitives.WriteUInt32BigEndian(digest[(i * 4)..], _state[i]);
            }
        }
        else
        {
            for (int i = 0; i < _state.Length; i++)
            {
                BinaryPrimitives.WriteUInt32LittleEndian(digest[(i * 4)..], _state[i]);
            }
        }
    }
